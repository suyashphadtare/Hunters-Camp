...
Property = Class.extend({
	...
	make: function() {
		...
		me.activate_prop = me.wrapper.page.add_field({
						fieldname: "activate_prop",
						label: __("Activate Property"),
						fieldtype: "Button",
						icon: "icon-share-sign"
		});
...
// end of code for unpublished prop
	...
	refresh: function() {
		...
		me.chk_box_class ="cb"
		me.render(frappe.route_options['data'],frappe.route_options['total_records'],me.chk_box_class);
	},
	...
	render: function(prop_list,total_records,chk_box_class) {
		var me = this;
		var current_page = 1;
		...
		this.changePage(1,numPages,this.prop_list,records_per_page,this.prop_list.length,flag='Normal',total_records,chk_box_class);
		

	},


	changePage: function(page,numPages,values,records_per_page,length,flag,total_records,chk_box_class)
	{	
		var me=this
		if(flag=='Normal'){
			$("#property").remove();
	    ...
	    me.show_user_property_table(page,numPages,values,records_per_page,length,flag,total_records,chk_box_class);
	...
	show_user_property_table: function(page,numPages,values,records_per_page,length,flag,total_records,chk_box_class) {
		...
		$.each(values, function(i, d) {
			...
			$("<ul id='mytab' class='nav nav-tabs' role='tablist' >\
				...
				<input type='checkbox' class="+chk_box_class+" />\
				</div></ul></div>\
			    </ul>\
			      ...
			      <div role='tabpanel' class='tab-pane fade active in' style='overflow:auto;height: 110px;' id='general"+""+i+"' aria-labelledby='home-tab'>\
			       <div class='col-md-6' style='background-color=#fafbfc;'>\
					        <div class='row row-id'>\
					          ...
					          <div class='row row-id'>\
							        <div class='col-md-6 row'>\
							      	<div class='row property-row'><b>Posted By :</b></div>\
								    </div>\
							       	<div class='col-md-6 row'>\
							       	<div class='row property-row' id='posted_by'></div>\
							       	</div>\
						</div>\
			       <div class='row row-id'>\
			        <div class='col-md-6 row'>\
		...
		$($(me.body).find("#"+d['property_id']+"")).find("#posted_by").append('<div class="row property-row">'+d['listed_by'] ? d['listed_by'] : ""+'</div>')
						...
						if(!r.exc) {
							if(r.message['data'].length>0){
								if(me.lead_management.$input.val().length != 0){
									...
									me.changePage(page,numPages,r.message['data'],records_per_page,r.message['data'].length,flag='Normal',total_records,me.chk_box_class);
						}
					}
					},
			});	
	...
	$( "#select_alert" ).change(function(){
		...
		if($("#select_alert").val()=='posting_date'){
			...
			me.changePage(page,numPages,me.property_data,records_per_page,me.property_data.length,flag='Sorting',total_records,me.chk_box_class);
		}
...
// Code added  by Arpit for creation of check box in unpublished property .
	...
	show_unique_properties:function(page,numPages,data,records_per_page,length,flag,total_records,chk_box_class){
		var me=this
		result=data
		return frappe.call({
				...
				if(r.message){
					if(Object.keys(r.message).length>0){
						...
						if(final_result.length>0){
							...
							me.changePage(page,numPages,final_result,records_per_page,final_result.length,flag='Normal',total_records,chk_box_class);
						}
						else{
							return frappe.call({
			  	...
			  	me.changePage(page,numPages,result,records_per_page,result.length,flag='Normal',total_records,chk_box_class);
			  }
			},
		});		
...
AgentPropertyShare = Class.extend({
	...
	get_table_html:function(){
		...
		$.each(this.prop_ids, function(i,property){
			select_html = '<select class="form-control prop_through has-error"><option></option><option>Direct</option><option>Through 1</option>\
							<option>Through 2</option><option>Through 3+</option></select>'
			doc_select = '<select class="has-error form-control doc_available"><option></option><option>Yes</option><option>No</option></select>'
			values = { "property_id":property["property_id"], "select":select_html, "doc_select":doc_select}
			// classs is added by arpit for mandatory filed for property through and doc available
			row = repl("<tr class='row'><td class='col-xs-4 p_id text-center' >%(property_id)s</td><td class='col-xs-4'><textarea type='text' class='comments'></textarea></td>\
				<td class='col-xs-2 has-error'>%(select)s</td><td class='col-xs-2 has-error'>%(doc_select)s</td></tr>", values)
			table_html += row
		})
		table_html += "</tbody>"
		return table_html
	...
	check_for_mandatory_fields:function(){
		var me = this
		var validate_field = me.validation_of_agent_fields()
		if (! $(me.dialog.body).find("textarea[data-fieldname='agents']").val()){
			...
			return false
		}else if(validate_field){
			//alert(JSON.stringify(validate_field)+"   "+ validate_field.length)
			for(var i =0 ; i <validate_field.length; i++){
				if (Object.keys(validate_field[i]).length != 0 ){
					frappe.msgprint(Object.keys(validate_field[i])+ " in row "+ (parseInt(i)+ 1)+" is mandatory for property sharing.");
 					return false
 					}
				}
	...
	// function start
	// function is aded by arpit  for validation for prop_through and doc_available
	validation_of_agent_fields:function(){
		comments_list = []
		var me = this
		$.each($(me.dialog.body).find("#prop_table tbody tr"), function(i, row){
			prop_dict = {}
			prop_through_val = $(row).find(".prop_through").val();
			if(!prop_through_val){
				//prop_dict["prop_through"] = $(row).index() + 1
				prop_dict["Property Through"] = i + 1
			}
			doc_available_val = $(row).find(".doc_available").val();
			if(!doc_available_val){
				prop_dict[" Doc Available"] = i + 1
			}
			//if(Object.keys(prop_dict).length != 0){				
				comments_list.push(prop_dict)
			//}
